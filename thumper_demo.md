...
This demo showcases capabilities of micro-ROS integrated with 6LoWPAN radio communication and presents its applying to the remote control of an unmanned mobile platform. As a reaserch platform the THUMPER robot was used rolled remotely with a joystick.
 
![tupec_intro](images/thumper_intro.png)
...
The developed solution gives the ability to roll the vehicle both forward and backward with adjustable speed and turn in any direction or stop at any time.

The basic elements of the project solution are shown on the diagram below.

## Operation diagram overview

![tupec_diagram](images/thumper_dgr.png
)

Both Olimex STM32-E407 boards are running NuttX and micro-ROS applications, which  can exchange data via UDP over 6LoWPAN. 
On the joystick's side, its current position is converted into the velocity command and then posted on a micro-ROS stack as a message sequence.
On the robot's side, the micro-ROS Olimex application subscribes these messages to supply a motor controller with obtained commands. 
...
## Topics
There are two micro-ROS nodes, the joystck's publisher and the robot's subscriber, which work on the topic `/velocity_cmd` - *the velocity for driving a RoboClaw motor controller*. 


## Hardware
The following is a list of the [demo hardware](doc/thumper_hardware.md):
- 1 x Wild Thumper 6WD Chassis, 
- 1 x RoboClaw 2x15A (V5E), 
- 2 x Olimex STM32-E407,
- 2 x Digilent Pmod RF2
...
How to build the Olimex STM32-E407 firmware with the micro-ROS applications is describe [here](doc/thumper_oli_apps.md).  
...
- Insert the ATUSB adapter into a USB port on PC and run the micro-ROS agent as described [here](doc/thumper_agent.md).

After a while, the micro-ROS publisher and subscriber will establish a connection with the agent and the vehicle can then be remotly driven with the joystick. To let the vehicle run forward / backward or change its speed, push the stick along X axis, to move leftward / rightward rotate the handle around Z axis.